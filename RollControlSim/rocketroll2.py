...
def rocketroll2(init_alpha,delay,withPID,Kp_rr,Ki_rr,Kd_rr,setPoint_rr,Kp_theta,Ki_theta,Kd_theta,setPoint_theta):
    #for storing data
    track_thetadotdot = []
    track_thetadot = []
    track_lift = []
    ...
    if(withPID==1):
        ...
        for i in range(delay):
            angle.append(init_alpha) #initialize array with value for initial canard angle
        p_rr=controller.setupRollRatePID(Kp_rr,Ki_rr,Kd_rr,setPoint_rr) #Set up roll rate PID
        p_theta=controller.setupThetaPID(Kp_theta,Ki_theta,Kd_theta,setPoint_theta) #Set up Theta PID
        alpha=init_alpha
    else:
        alpha=init_alpha
